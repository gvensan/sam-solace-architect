...
**File:** `plugins/solace-architect-discovery/config.yaml` (SAM `apps:` block — `app_module: solace_agent_mesh.agent.sac.app`). Role-specific prompt mirrored in `src/solace_architect_discovery/prompt.py` for test inspection; the two are kept in sync by `test_prompt_module_matches_config` (Phase 1, first-landed agent — Decision 85).

**Purpose:** Refines a partially-filled intake into a complete discovery brief by following up on **gaps, ambiguities, and missing decisions only**. The V2 intake form captures ~90% of what V1's cold-start interview asked, so Discovery's V2 job is gap-following: read the submitted `intake.json`, classify each field as complete / ambiguous / missing-blocking / missing-advisory, and ask only about the unclear ones. Performs reference architecture pattern matching. Verifies each named backend against the Integration Hub catalog. Records gaps as `open-items` (blocking vs advisory). Produces `discovery/discovery-brief.yaml` + `discovery/discovery-summary.md`.
...
| 85 | First-agent build = SADiscoveryAgent | **SADiscoveryAgent ships first**, ahead of SAOrchestratorAgent. WebUI entrypoint's `default_agent_name` points directly at Discovery; when Orchestrator lands later, the default switches to Orchestrator and Discovery becomes a peer reachable via A2A delegation. Agent uses SAM's universal `app_module: solace_agent_mesh.agent.sac.app` — no plugin-side `app.py`. Role-specific prompt is inlined in `app_config.instruction` AND mirrored in `src/<pkg>/prompt.py` for test inspection; `test_prompt_module_matches_config` keeps them in sync. Telemetry capture (Decision 84's `after_model_callback` registration) deferred to a separate follow-up — SAM doesn't expose a config hook for extra callbacks, so we'll either monkey-patch `TaskExecutionContext.record_token_usage` or wrap `SamAgentApp`. Discovery itself works without capture today; the Usage view's empty state is honest. | Building Orchestrator first violates "smallest demoable end-to-end slice" — orchestrator alone has nothing to orchestrate. Discovery is the highest-demo-value first agent (intake form → real conversation → enriched brief) and exercises the load_preamble / load_grounding / query_integration_hub plumbing that proves Phase-1 tools work in anger. |
