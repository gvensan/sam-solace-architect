...
- **Always persist via `write_artifact` / `append_artifact` — never a fenced "artifact block".** Those tools write into the engagement's store, where the project view and downstream phases read. Do NOT "save" a file by emitting a fenced artifact block or an `«artifact…»` / `save_artifact:` embed marker in your response text — SAM's stream parser routes those to a *different* store the engagement can't read, so the artifact silently vanishes (it won't appear in the project and the next phase won't find it). For large content, **chunk with `append_artifact`** (below) — never fall back to a fenced block.
